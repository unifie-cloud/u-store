...
const commonOptions: any = {
  ...
  scales: {
    x: {
      ticks: { autoSkip: true, maxRotation: 0, padding: 12, minRotation: 0 },
    },
  },
};
...
const AreaChartRenderer = ({
  ...
  const options: any = {
    ...commonOptions,
    scales: { ...commonOptions.scales, y: { stacked: true } },
  };
  // const getElementAtEvent = useDrilldownCallback({
  //   datasets: data.datasets,
  //   labels: data.labels,
...
const formatTableData = (columns, data) => {
  ...
  function formatValue(value, fV: { type: any; format: any }) {
    ...
    if (fV?.type === 'number' && fV?.format === 'percent')
...
const renderChartWithModal = (props) => {
  ...
  return (
    <>
      <ChartModal originalChartView={originalChartView}>
        {renderChart(props)}
      </ChartModal>
    </>
  );
...
export const ChartRenderer = (props) => {
  ...
  const key =
    JSON.stringify(props.vizState.query) +
    JSON.stringify(props.vizState.chartType) +
    JSON.stringify(props.vizState.pivotConfig);
  return (
    <QueryRenderer
      key={key}
